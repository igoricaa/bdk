...
// Query: *[_type in ["post", "lawyer", "industry", "practice", "foreignDesk"]] {    _id,    _type,    title,    name,    "slug": slug.current,    "details": select(_type == "lawyer" => name, null),    "content": pt::text(content)  }
...
// Query: *[_type == "lawyer" && ($category == "all" || category->slug.current == $category)] {    _id,    name,    title,    picture,    slug,    contactInfo {      linkedin    },    areasOfExpertise  }
...
// Query: *[_type == "author" && slug.current == $slug][0] {    _id,    name,    slug,    type,    "lawyerDetails": lawyer->{      title,      picture,      bio,      contactInfo {        email,        phone,        linkedin      }    },    "posts": *[      _type == "post" &&      status == "publish" &&      references(^._id)    ] | order(date desc)[0...10] {      _id,      title,      slug,      date,      featuredMedia,      categories[]->{        _id,        name,        slug      },      authors[]->{        _id,        name,        slug      }    }  }
...
// Query: *[_type == "post" && status == "publish" && references(*[_type=="category" && slug.current == $categorySlug]._id) && date match $year + "-*"] | order(date desc)[$start...$end] {    _id,    title,    slug,    date,    featuredMedia,    categories[]->{      _id,      name,      slug    }  }
...
// Query: {  "lawyers": *[_type == "lawyer" && (!defined(isVisible) || isVisible == true)]{    name,    title,    picture,    slug,    category->{      _id,      title,      slug,      order,      orderedLawyers[]->{        _id,        slug      }    },    contactInfo,    areasOfExpertise  }}
...
// Query: {  "categories": *[_type == "lawyerCategory"] | order(order asc) {    _id,    title,    slug,    order,    "orderedLawyers": orderedLawyers[]->{       _id,      name,      title,      picture,      slug,      contactInfo {        linkedin      },      isVisible,      areasOfExpertise    }  }}
...
// Query: {  "latestBlogPosts": *[_type == "post" && status == "publish" && count((categories[]._ref)[@ in $latestBlogPostsRefs]) > 0] | order(date desc)[0...4]{    _id,    title,    slug,    date,  },  "bdkInsights": *[_type == "post" && status == "publish" && count((categories[]._ref)[@ in $bdkInsightsRefs]) > 0] | order(date desc)[0...4]{    _id,    title,    slug,    date,  },}
...
// Query: {  "posts": *[_type == "post" && status == "publish" && ($slug == "all" || $slug == null || references(*[_type=="category" && slug.current == $slug]._id))] | order(date desc)[0...9] {    _id,    title,    slug,    date,    featuredMedia,    authors[]->{      _id,      name,      slug    },    categories[]->{      _id,      name,      slug    }  }}
...
// Variable: CATEGORIES_BY_YEAR_QUERY
// Query: *[_type == "category" && count(*[_type == "post" && status == "publish" && references(^._id) && string(date) match $year + "*"]) > 0] {    _id,    name,    slug  } | order(name asc)
export type CATEGORIES_BY_YEAR_QUERYResult = Array<{
  _id: string;
  name: string;
  slug: Slug;
}>;

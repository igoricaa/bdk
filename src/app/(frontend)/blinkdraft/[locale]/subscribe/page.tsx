...
const SubscribePage = async ({
  ...
  const { blinkdraftSubscriptionForm } =
    await getBlinkdraftSubscriptionFormData(locale);

  if (!blinkdraftSubscriptionForm) {
    notFound();
  }
    ...
    <main className='pt-header bg-light-blue-bg'>
      <div className='py-25'>
        <h1 className='text-dark-blue text-center text-3xl md:text-4xl lg:text-5xl 2xl:text-6xl md:max-lg:px-side'>
          {blinkdraftSubscriptionForm.title}
        </h1>

        <SubscribeForm
          formData={
            blinkdraftSubscriptionForm as BLINKDRAFT_SUBSCRIPTION_FORM_QUERYResult['blinkdraftSubscriptionForm']
          }
